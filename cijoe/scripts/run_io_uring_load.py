"""
Plain-liburing loader measurement.

Drops the OS page cache before each iteration, then runs the standalone
io-uring-load tool. This binary bypasses xNVMe entirely and just uses
liburing directly on the file, so its numbers bracket what the OS
storage stack can deliver on this box before any xNVMe overhead.
"""
...
def add_args(parser: ArgumentParser):
    parser.add_argument("--io_uring_load_bin", type=str,
                        default="/usr/local/bin/io-uring-load")
...
def main(args, cijoe):
    ...
    for io_size in sizes:
        ...
        log.info(f"=== io-uring-load io_size={io_size or 'default'} ===")
        ...
        for i in range(args.iterations):
            ...
            err, _ = cijoe.run(cmd)
            if err:
                log.error(f"io_size={io_size} iter {i}: io-uring-load exited nonzero")
                return err
